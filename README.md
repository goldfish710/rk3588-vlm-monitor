...
docs/
├── 项目交接说明书.md     (★接手项目先读这个:定位/架构/状态/待办/协作约定)
├── 公网MQTT部署清单.md   (云端 EMQX + 板端 + 网页客户端,含验证与排错)
├── 技术文档.md         (21 章,面试学习)
├── 简历-项目经历.md     (可直接粘贴版)
├── 验收清单.md          (三幕式板端验收)
├── 开发日志.md         (开发全记录)
├── C++教学/  LLM与VLM入门教学.md / LLM与VLM纯小白入门.md
└── 面试讲稿-项目难点故事.md
```

## 关键指标(板端实测)

- 视频:双码流 30fps,常驻 CPU ~2%(watchload 实测,编码/旋转缩放全硬件化)
- 视觉 AI:yolov8n-pose 640 推理 22-24ms(15fps 抽帧,多核无收益已实测)
- 语音 ASR:RTF 0.087(绑核优化 -15%)
- 跌倒复核链:候选→分级报警 ~3.5-5.5s 预算(vision 2s + 短答复 1.5s;实测数字见验收清单)
- 对话首句延迟:~3s(VLM 流式 + Piper 逐句流水线;断网全本地闭环)
- 长稳:水位有界、退出干净(SIGINT/SIGTERM 全资源回收,offline retained 发布)

## 许可与说明

个人学习项目。云端 API key 明文存板端配置、TLS 跳过证书校验均为个人场景权衡
(技术文档 17 章有说明);商用需服务端中转与证书校验。
