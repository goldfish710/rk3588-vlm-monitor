...
// 前置声明（定义在文件后部）
static int post_process_single_output(rknn_app_context_t *app_ctx, void *outputs,
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold,
    ...
    // 单输出模型（ultralytics 8.x 导出：[1,56,N]）走独立路径
    if (app_ctx->io_num.n_output == 1) {
        return post_process_single_output(app_ctx, outputs, letter_box,
    ...
    for (int i = 0; i < validCount; ++i) {
        ...
        for (int j = 0; j < 17; ++j) {
            float kpx, kpy, kpc;
            // 按关键点输出张量的实际类型解析：
            // 旧模型(v1)：FLOAT16（不量化）；2.2.1 重转模型：INT8（需反量化）
            switch (app_ctx->output_attrs[3].type) {
                case RKNN_TENSOR_INT8: {
                    const int8_t *kbuf = (const int8_t *)_outputs[3].buf;
                    const rknn_tensor_attr &ka = app_ctx->output_attrs[3];
                    kpx = deqnt_affine_to_f32(kbuf[j*3*8400 + 0*8400 + keypoints_index], ka.zp, ka.scale);
                    kpy = deqnt_affine_to_f32(kbuf[j*3*8400 + 1*8400 + keypoints_index], ka.zp, ka.scale);
                    kpc = deqnt_affine_to_f32(kbuf[j*3*8400 + 2*8400 + keypoints_index], ka.zp, ka.scale);
                    break;
                }
                case RKNN_TENSOR_UINT8: {
                    const uint8_t *kbuf = (const uint8_t *)_outputs[3].buf;
                    const rknn_tensor_attr &ka = app_ctx->output_attrs[3];
                    kpx = deqnt_affine_u8_to_f32(kbuf[j*3*8400 + 0*8400 + keypoints_index], ka.zp, ka.scale);
                    kpy = deqnt_affine_u8_to_f32(kbuf[j*3*8400 + 1*8400 + keypoints_index], ka.zp, ka.scale);
                    kpc = deqnt_affine_u8_to_f32(kbuf[j*3*8400 + 2*8400 + keypoints_index], ka.zp, ka.scale);
                    break;
                }
                case RKNN_TENSOR_FLOAT16: {
                    kpx = (float)((rknpu2::float16 *)_outputs[3].buf)[j*3*8400 + 0*8400 + keypoints_index];
                    kpy = (float)((rknpu2::float16 *)_outputs[3].buf)[j*3*8400 + 1*8400 + keypoints_index];
                    kpc = (float)((rknpu2::float16 *)_outputs[3].buf)[j*3*8400 + 2*8400 + keypoints_index];
                    break;
                }
                default: {   // FLOAT32
                    const float *kbuf = (const float *)_outputs[3].buf;
                    kpx = kbuf[j*3*8400 + 0*8400 + keypoints_index];
                    kpy = kbuf[j*3*8400 + 1*8400 + keypoints_index];
                    kpc = kbuf[j*3*8400 + 2*8400 + keypoints_index];
                    break;
                }
            }
            od_results->results[last_count].keypoints[j][0] = (kpx - letter_box->x_pad) / letter_box->scale;
            od_results->results[last_count].keypoints[j][1] = (kpy - letter_box->y_pad) / letter_box->scale;
            od_results->results[last_count].keypoints[j][2] = kpc;
        }
...
// =====================================================
// 单输出模型后处理（ultralytics 8.x 导出的 pose 格式）
//
// output0 [1, 56, N]（N = 320/640 输入的 anchor 数）：
//   通道 0:3  = box xywh（绝对像素坐标，DFL 解码已内嵌模型）
//   通道 4    = conf（已 sigmoid，0~1）
//   通道 5:55 = 17 关键点 × (x, y, vis)，绝对坐标，vis 已 sigmoid
//
// 与旧 4 输出格式（3 个检测头 + Concat_6 关键点）完全不同的路径，
// 后处理更简单：无 DFL 解码、无 per-head grid 循环。
// =====================================================
static int post_process_single_output(rknn_app_context_t *app_ctx, void *outputs,
                                      letterbox_t *letter_box, float conf_threshold,
                                      float nms_threshold, object_detect_result_list *od_results)
{
    ...
    int anchors = attr.dims[2];                       // N
    if (attr.dims[1] != 56 || anchors <= 0) {
        printf("single-output postprocess: 意外输出形状 [%d,%d,%d]\n",
               attr.dims[0], attr.dims[1], attr.dims[2]);
        return -1;
    }

    rknn_output *_outputs = (rknn_output *)outputs;
    // 统一转 float（float16 输出由调用方 want_float=true 或此处转换）
    std::vector<float> buf;
    const float *o;
    if (attr.type == RKNN_TENSOR_FLOAT16) {
        buf.resize(56 * (size_t)anchors);
        const rknpu2::float16 *src = (const rknpu2::float16 *)_outputs[0].buf;
        for (size_t i = 0; i < buf.size(); i++) buf[i] = (float)src[i];
        o = buf.data();
    } else if (attr.type == RKNN_TENSOR_FLOAT32) {
        o = (const float *)_outputs[0].buf;
    } else {
        printf("single-output postprocess: 不支持输出类型 %d\n", attr.type);
        return -1;
    }
    ...
    int last_count = 0;
    for (size_t i = 0; i < cands.size() && last_count < OBJ_NUMB_MAX_SIZE; i++) {
        if (!keep[i]) continue;
        const Cand &ci = cands[i];
        // letterbox 反变换：绝对坐标 → 原图坐标
        float x1 = (ci.cx - ci.w / 2 - letter_box->x_pad) / letter_box->scale;
        float y1 = (ci.cy - ci.h / 2 - letter_box->y_pad) / letter_box->scale;
        float x2 = (ci.cx + ci.w / 2 - letter_box->x_pad) / letter_box->scale;
        float y2 = (ci.cy + ci.h / 2 - letter_box->y_pad) / letter_box->scale;

        object_detect_result &res = od_results->results[last_count];
        res.box.left = (int)x1;
        res.box.top = (int)y1;
        res.box.right = (int)x2;
        res.box.bottom = (int)y2;
        res.prop = ci.conf;
        res.cls_id = 0;

        for (int k = 0; k < 17; k++) {
            res.keypoints[k][0] = (o[(5 + 3 * k + 0) * anchors + ci.idx] - letter_box->x_pad) / letter_box->scale;
            res.keypoints[k][1] = (o[(5 + 3 * k + 1) * anchors + ci.idx] - letter_box->y_pad) / letter_box->scale;
            res.keypoints[k][2] = o[(5 + 3 * k + 2) * anchors + ci.idx];
        }
        last_count++;
        ...
        for (size_t j = i + 1; j < cands.size(); j++) {
            ...
            float ix1 = std::max(ci.cx - ci.w / 2, cj.cx - cj.w / 2);
            float iy1 = std::max(ci.cy - ci.h / 2, cj.cy - cj.h / 2);
            float ix2 = std::min(ci.cx + ci.w / 2, cj.cx + cj.w / 2);
            float iy2 = std::min(ci.cy + ci.h / 2, cj.cy + cj.h / 2);
            float iw = std::max(0.0f, ix2 - ix1);
            float ih = std::max(0.0f, iy2 - iy1);
            float inter = iw * ih;
            float uni = ci.w * ci.h + cj.w * cj.h - inter;
            if (uni > 0.0f && inter / uni > nms_threshold) keep[j] = false;
        }
    }
    od_results->count = last_count;
    return 0;
